...
fn run_shell(registry: &CommandRegistry, command_name: &str, options: ExecutionOptions<'_>) -> i32 {
    ...
    if !is_raw_path(command_name) && registry.get(command_name).is_none() {
        eprintln!("Error: Command '{command_name}' not found");
        eprintln!("Run 'envoy --list' to see available commands");
        return emission()
            .with_error_category(ErrorCategory::CommandNotFound)
    ...
    let (env_map, command) = match prepare_env(
        command_name,
        registry,
        options.bundles,
        options.inherit_env,
    ...
    println!(
        "Entering shell inside '{command_name}''s resolved environment. Type 'exit' to return."
    );
