...
from ._exceptions import ExecutionError, PostRunError, PreRunError
...
class ApplicationWrapper:
    ...
    def __init__(self, config: WrapperConfig):
        ...
        self._env_manager = EnvironmentManager(
            inherit_env=config.inherit_env, allowlist=config.env_allowlist
        )
        self._executor = ProcessExecutor(
            stream_output=config.stream_output, onOutput=config.onOutput, onError=config.onError
        )
    ...
    def run(self) -> ExecutionResult:
        ...
        result = ExecutionResult(return_code=-1, command=[])
        ...
        try:
            ...
            env = self._env_manager.prepareEnvironment(
                env_files=self.config.env_files, env=self.config.env
            )
...
def createWrapper(
    ...
    *args: str,
    ...
    # preRun/postRun: public callback API, intentional camelCase
    preRun: Callable[[], None] | None = None,  # noqa: N803
    postRun: Callable[[ExecutionResult], None] | None = None,  # noqa: N803
    **kwargs,
) -> ApplicationWrapper:
