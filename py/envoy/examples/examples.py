...
from gt.app.wrapper import ApplicationWrapper, ExecutionResult, WrapperConfig, createWrapper
...
def example_basic():
    ...
    config = WrapperConfig(executable="python", args=["--version"], capture_output=True)
...
def example_with_callbacks():
    ...
    config = WrapperConfig(
        ...
        args=[
            "-c",
            "print('Line 1'); print('Line 2'); import sys; print('Error!', file=sys.stderr)",
        ],
        onStart=onStart,
        onOutput=onOutput,
        onError=onError,
...
def example_convenience_function():
    ...
    wrapper = createWrapper(
        "python", "-c", "print('Hello from wrapper!')", capture_output=True, timeout=5.0
    )
...
def example_list_paths():
    ...
    env_with_lists = {
        "PYTHONPATH": ["R:/repo/project1/py", "R:/repo/project2/py", "R:/repo/project3/py"],
        "PATH": ["C:/Python312", "C:/Python312/Scripts", "R:/repo/tools/bin"],
        "+=CUSTOM_PATH": ["C:/extra/path1", "C:/extra/path2"],
...
""",
    ...
    # Append operator
    append_op_env = {"+=MY_CUSTOM_PATH": "C:\\appended\\with\\operator"}
    append_op_file = temp_dir / "append_op_env.json"
    with open(append_op_file, 'w') as f:
        json.dump(append_op_env, f, indent=2)

    # Prepend operator
    prepend_op_env = {"^=MY_CUSTOM_PATH": "C:\\prepended\\with\\operator"}
...
def example_special_variables():
    ...
    env_config = {
        "+=PYTHONPATH": ["{$__BUNDLE__}/py", "{$__BUNDLE__}/lib/python"],
        "+=PATH": "{$__BUNDLE__}/bin",
        "APP_ROOT": "{$__BUNDLE__}",
