...
def test_timeout():
    ...
    print(
        f"  ✅ Timeout test passed: timed_out={result.timed_out}, time={result.execution_time:.2f}s"
    )
...
def test_callbacks():
    ...
    print(
        f"  ✅ Callbacks test passed: {len(events['output'])} stdout, {len(events['error'])} stderr"
    )
