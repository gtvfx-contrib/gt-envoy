...
Carried over unchanged from the pre-Rust-migration implementation
(``py/envoy/testing.py``): this module only manipulates process
environment variables via :mod:`os` and :mod:`pathlib`, with no dependency
on any envoy-core internals, so there is nothing to port -- it works
identically whether the compiled ``envoy._envoy`` extension or the pure
Python implementation backs the rest of the package.
